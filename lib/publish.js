...
function publish (args, isRetry, cb) {
  ...
  var progress = log.newItem('publish', 6, 6)
  ...
  readJson(path.resolve(arg, 'package.json'), function (er, data) {
    ...
    if (er) return cacheAddPublish(arg, false, isRetry, progress, cb)
    else cacheAddPublish(arg, true, isRetry, progress, cb)
  })
}
...
// in the cache dir.
function cacheAddPublish (dir, didPre, isRetry, progress, cb) {
  npm.commands.cache.add(dir, null, null, false, function (er, data) {
    if (er) return cb(er)
    progress.completeWork(1)

    function completedOne (cb) {
      progress.completeWork(1)
      cb()
    }
    ...
    chain(
      [
        !didPre && [lifecycle, data, 'prepublish', cachedir], [completedOne],
        [publish_, dir, data, isRetry, cachedir, log.newItem('publish', 1)],
        [lifecycle, data, 'publish', didPre ? dir : cachedir], [completedOne],
        [lifecycle, data, 'postpublish', didPre ? dir : cachedir], [completedOne]
      ],
      function () {
...
function publish_ (arg, data, isRetry, cachedir, progress, cb) {
