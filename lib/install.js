...
function Installer (where, dryrun, args) {
  ...
  this.where = where
  this.node_modules = path.resolve(this.where, 'node_modules')
  this.staging = path.resolve(this.node_modules, '.staging')
  this.dryrun = dryrun
  this.args = args
...
Installer.prototype.run = function (cb) {
  ...
  installSteps.push(
    ...
    [this, this.preinstallTopLevel, log.newGroup('preinstall')],
  ...
  if (!this.dryrun) {
    ...
    postInstallSteps.push(
      ...
      [this, this.rollbackFailedOptional, this.staging, this.todo],
      [this, this.finishTracker, 'rollbackFailedOptional'],
      [this, this.commit, this.staging, this.todo],
...
Installer.prototype.preinstallTopLevel = function (log, cb) {
  doOneAction('preinstall', this.staging, this.currentTree, log, cb)
}
...
Installer.prototype.executeActions = function (cb) {
  ...
  cb = unlockCB(this.node_modules, '.staging', cb)

  steps.push(
    [doSerialActions, 'global-install', this.staging, todo, trackLifecycle.newGroup('global-install')],
    [doParallelActions, 'fetch', this.staging, todo, cg.newGroup('fetch', 10)],
    [lock, this.node_modules, '.staging'],
    [rimraf, this.staging],
    [mkdirp, this.staging],
    [doParallelActions, 'extract', this.staging, todo, cg.newGroup('extract', 10)],
    [doParallelActions, 'preinstall', this.staging, todo, trackLifecycle.newGroup('preinstall')],
    [doReverseSerialActions, 'remove', this.staging, todo, cg.newGroup('remove')],
    [doSerialActions, 'move', this.staging, todo, cg.newGroup('move')],
    [doSerialActions, 'finalize', this.staging, todo, cg.newGroup('finalize')],
    [doSerialActions, 'build', this.staging, todo, trackLifecycle.newGroup('build')],
    [doSerialActions, 'global-link', this.staging, todo, trackLifecycle.newGroup('global-link')],
    [doParallelActions, 'update-linked', this.staging, todo, trackLifecycle.newGroup('update-linked')],
    [doSerialActions, 'install', this.staging, todo, trackLifecycle.newGroup('install')],
    [doSerialActions, 'postinstall', this.staging, todo, trackLifecycle.newGroup('postinstall')])
  if (this.npat) {
    steps.push(
      [doParallelActions, 'test', this.staging, todo, trackLifecycle.newGroup('npat')])
  }

  var self = this
  chain(steps, function (er) {
    if (!er || self.rollback) {
      rimraf(self.staging, function () { cb(er) })
    } else {
      cb(er)
    }
  })
